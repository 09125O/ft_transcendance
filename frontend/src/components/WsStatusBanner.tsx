...
const LABELS: Record<WsConnectionState, { label: string; tone: string } | null> = {
  idle: null,
  connected: null,
  connecting: { label: "Connexion au serveur temps réel…", tone: "bg-text/10" },
  reconnecting: { label: "Reconnexion en cours…", tone: "bg-urgency/20" },
  disconnected: {
...
export default function WsStatusBanner() {
  ...
  return (
    <div
      className={`fixed bottom-4 left-1/2 z-50 -translate-x-1/2 rounded-full px-4 py-2 text-sm text-text shadow-lg ${meta.tone}`}
      role="status"
    >
      {meta.label}
    </div>
