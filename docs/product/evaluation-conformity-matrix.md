...
Derniere verification documentaire: 2026-05-13, par lecture statique de l'implementation courante et des contrats versionnes.

Derniere verification executable referencee par le repo: smoke tests, browser test et workflow CI vert documentes dans les preuves de soutenance.
...
| Notifications create/update/delete | Minor | Fait | `GET /notifications`, `PATCH /notifications/:id/read`, `PATCH /notifications/read-all`, `DELETE /notifications/:id`, event `notification:new`, UI dans `frontend/src/pages/FriendsPage.tsx`, test `backend/test/scenarios/social-integration-test.mjs` | - | Frontend | lot notifications 2026-04-18 |
