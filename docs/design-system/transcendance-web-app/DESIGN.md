...
## 0. Project Overrides for Quiz Arena (Priority Rules)

These rules override the generic PlayStation profile for this repository.

- Product context: this app is a live multiplayer quiz platform, not a retail storefront.
- Keep the visual DNA: PlayStation Blue + dark/white surface alternation + lightweight display typography + signature hover motion.
- Remove retail semantics: avoid "buy", "cart", "store", "sale", "subscribe", and all purchase-oriented language in UI or prompts.
- Interaction priorities must match gameplay loops:
  - pre-match: create/join room, ready states, participant visibility, host controls
  - in-game: question readability, answer selection clarity, timer pressure, anti-misclick spacing
  - post-game: ranking legibility, score deltas, replay or lobby return actions
- Required UI states for each major screen: loading, empty, error, ready.
- Preserve validated French copy already accepted by product review. Improve hierarchy before rewriting wording.
- Orange usage (`#d53b00`) is not a commerce cue here. Reserve it only for high-urgency signals (for example final seconds countdown), never as a default CTA fill.
- Default CTA hierarchy:
  - Primary action: `#0070cc` at rest with PlayStation hover signature
  - Secondary action: outline/neutral
  - Destructive action: warning red family (`#c81b3a`) with explicit confirmation
...
- Urgency orange `#d53b00` used only for critical countdown or urgent host warnings
...
### Semantic & Urgency
- **Urgency Orange** (`#d53b00`): Reserved for high-urgency gameplay moments (e.g. countdown danger zone, host-only urgent warning). Never use as a default CTA.
- **Urgency Orange Active** (`#aa2f00`): The pressed/active state of urgency UI elements.
...
| Button / Emphasized | SST | 18px / 1.13rem | 700 | 1.25 | 0.45px | Higher-emphasis CTAs (host actions, confirm transitions) |
...
**Urgency Variant (Limited Use)**
- Background: `#d53b00` (Urgency Orange)
- Text: `#ffffff`, SST 18px / 700 / 0.45px tracking
- Border radius: `999px` — pill
- Used only in high-urgency gameplay contexts, never as a default primary CTA
- Active: background darkens to `#aa2f00`
- Hover: follows the cyan-invert rule like all other buttons
...
- Padding ~8px 16px
- Radius: `999px`
- Used inside room cards for "Rejoindre" / "Pret" / "Details" mini actions
...
### Platform Pill (Distinctive)
- Background: `#ffffff`
- Text: `#000000`, SST 14px / 500
- Padding: `14px 18px`
- Radius: `9999px` — full pill
- A neutral pill tag that sits next to room or quiz cards to label mode/platform ("Quiz", "Live", "Ranked"). White-on-dark contrast.
...
- **Do** use full-pill radius (`999px`) on primary and urgency buttons.
- **Do** reserve PlayStation Cyan (`#1eaedb`) exclusively for hover, focus, and active states — never as a resting background.
- **Do** use Urgency Orange (`#d53b00`) only for time-critical warnings or host-critical urgent actions.
...
- **Don't** introduce warm colors outside urgency or destructive contexts. No random yellow highlights or green success pills.
...
- **Urgency Accent (Limited Use)**: "Urgency Orange (`#d53b00`)"
...
3. *"Build a room tile: 16:9 thumbnail with 12px border radius, feather-weight `rgba(0, 0, 0, 0.08) 0 5px 9px 0` drop shadow, a 14px SST 700 room name, a 12px SST 500 status chip (waiting/playing/finished), and a mini 14px / 700 / 0.324px tracking primary CTA in PlayStation Blue labeled 'Rejoindre'."*
4. *"Create a countdown urgency badge for the last 5 seconds: `#d53b00` fill, `#ffffff` text in SST 18px / 700 / 0.45px tracking, 999px radius, compact horizontal padding, subtle pulse animation under 300ms, and no usage outside critical timer states."*
5. *"Design a white content panel between dark hero sections for post-match results: `#ffffff` background with the subtle `#ffffff → #f5f7fa` light section gradient, 24px border radius, 48px interior padding, feather-weight `rgba(0, 0, 0, 0.06) 0 5px 9px 0` elevation, a 35px SST 300 headline, ranking rows with clear score deltas, and one primary CTA to return to lobby."*
...
4. **Audit color sprawl.** Only PlayStation Blue (`#0070cc`), Cyan (`#1eaedb`), Urgency Orange (`#d53b00`, limited use), and the declared grays/blacks/whites should appear in chrome. If you see any other hue, correct it.
...
9. **Audit gameplay clarity.** Pre-match, in-game, and results states must be visually distinct at first glance, with no single-screen overload.
